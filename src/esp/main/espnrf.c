...
//
	...
	strcat(llwtd,"\",\"mdl\":\"espNRF\",\"sw\":\"");
	strcat(llwtd,AP_VER);
	if (wbuff[0]) {
	strcat(llwtd,"\",\"cu\":\"http://");
	strcat(llwtd,wbuff);
	}
	strcat(llwtd,"\",\"mf\":\"Espressif & Nordic Semiconductor\"},");
	strcat(llwtd,"\"dev_cla\":\"signal_strength\",\"stat_cla\":\"measurement\",\"stat_t\":\"");
	strcat(llwtd,MQTT_BASE_TOPIC);
	strcat(llwtd,"/rssi\",\"unit_of_meas\":\"dBm\",\"avty_t\":\"");
	strcat(llwtd,MQTT_BASE_TOPIC);
	strcat(llwtd,"/status\"}");
...
//
	...
	strcat(llwtd,"\"cmd_t\":\"");
	strcat(llwtd,MQTT_BASE_TOPIC);
	strcat(llwtd,"/status\",\"payload_press\":\"restart\",\"avty_t\":\"");
	strcat(llwtd,MQTT_BASE_TOPIC);
	strcat(llwtd,"/status\"}");
	esp_mqtt_client_publish(mqttclient, llwtt, llwtd, 0, 1, 1);
//
	...
	strcat(llwtd,"\",\"mf\":\"Espressif & Nordic Semiconductor\"},\"cmd_t\":\"");
	strcat(llwtd,MQTT_BASE_TOPIC);
	strcat(llwtd,"/");
	strcat(llwtd,NAMONOF1);
	strcat(llwtd,"\",\"stat_t\":\"");
...
//
	...
	strcat(llwtd,"\",\"mf\":\"Espressif & Nordic Semiconductor\"},\"stat_t\":\"");
	strcat(llwtd,MQTT_BASE_TOPIC);
	strcat(llwtd,"/");
	strcat(llwtd,NAMONOF1);
	strcat(llwtd,"/count\",\"avty_t\":\"");
	strcat(llwtd,MQTT_BASE_TOPIC);
	strcat(llwtd,"/status\"}");
	esp_mqtt_client_publish(mqttclient, llwtt, llwtd, 0, 1, 1);
//
	...
	strcat(llwtd,"/");
	strcat(llwtd,NAMONOF1);
	strcat(llwtd,"/set\",\"set_pos_t\":\"");
	strcat(llwtd,MQTT_BASE_TOPIC);
	strcat(llwtd,"/");
	strcat(llwtd,NAMONOF1);
	strcat(llwtd,"/set\",\"pos_t\":\"");
	strcat(llwtd,MQTT_BASE_TOPIC);
	strcat(llwtd,"/");
	strcat(llwtd,NAMONOF1);
//	strcat(llwtd,"/position\",\"position_close\":0,\"position_open\":100,\"stat_t\":\"");
	strcat(llwtd,"/position\",\"stat_t\":\"");
	strcat(llwtd,MQTT_BASE_TOPIC);
	strcat(llwtd,"/");
	strcat(llwtd,NAMONOF1);
	strcat(llwtd,"/state\",\"avty_t\":\"");
	strcat(llwtd,MQTT_BASE_TOPIC);
	strcat(llwtd,"/status\"}");
	esp_mqtt_client_publish(mqttclient, llwtt, llwtd, 0, 1, 1);
//
...
*/
		...
		if ((lvout ^ gpio_get_level(RELAY1)) || (lvout ^ gpio_get_level(RELAY2))) s_retry_num = 1;
		}
#endif
//	if ((!floop && (s_retry_num < WIFI_MAXIMUM_RETRY)) || (floop && (s_retry_num < (WIFI_MAXIMUM_RETRY << 6)))) {
...
//5
	...
	strcat(bufd,ttbuff);
	strcat(bufd,"\",\"mdl\":\"NRF24LE1\",\"via_device\":\"espNRF_");
	strcat(bufd,tESP8266Addr);
	strcat(bufd,"\",\"mf\":\"Nordic Semiconductor\"},\"cmd_t\":\"");
	strcat(bufd,MQTT_BASE_TOPIC);
	strcat(bufd,"/");
	strcat(bufd,MQTT_DATA);
	strcat(bufd,"\",\"stat_t\":\"");
...
//
	...
	strcat(bufd,"\",\"mdl\":\"NRF24LE1\",\"via_device\":\"espNRF_");
	strcat(bufd,tESP8266Addr);
	strcat(bufd,"\",\"mf\":\"Nordic Semiconductor\"},\"dev_cla\":\"temperature\",\"stat_cla\":\"measurement\",\"stat_t\":\"");
	strcat(bufd,MQTT_BASE_TOPIC);
	strcat(bufd,"/");
	strcat(bufd,MQTT_DATA);
	strcat(bufd,"\",\"unit_of_meas\":\"\xc2\xb0\x43\",\"avty\":[{\"t\":\"");
	strcat(bufd,MQTT_BASE_TOPIC);
	strcat(bufd,"/status");
	if (dev < 10) strcat(bufd,"0");
	itoa(dev,ttbuff,10);
...
//
	...
	strcat(bufd,"\",\"mdl\":\"NRF24LE1\",\"via_device\":\"espNRF_");
	strcat(bufd,tESP8266Addr);
	strcat(bufd,"\",\"mf\":\"Nordic Semiconductor\"},\"dev_cla\":\"voltage\",\"stat_cla\":\"measurement\",\"stat_t\":\"");
	strcat(bufd,MQTT_BASE_TOPIC);
	strcat(bufd,"/");
	strcat(bufd,MQTT_DATA);
	strcat(bufd,"\",\"unit_of_meas\":\"V\",\"avty\":[{\"t\":\"");
	strcat(bufd,MQTT_BASE_TOPIC);
	strcat(bufd,"/status");
	if (dev < 10) strcat(bufd,"0");
	itoa(dev,ttbuff,10);
...
*/
	...
	strcat(bsend,"> Hass Discovery");
#ifdef EXT_MODE
	strcat(bsend,"&emsp;<input type=\"checkbox\" name=\"chk2\" value=\"2\"");
	if (ftrufal) strcat(bsend,"checked");
        strcat(bsend,"> \"true/false\" Response(\"ON/OFF\" only if Hass Discovery)");
#endif
	strcat(bsend,"<br/><h3>System Setting</h3><br/><input name=\"sespnum\" type=\"number\" value=\"");
	itoa(espnrfnum,buff,10);
	strcat(bsend,buff);
...
#ifdef EXT_MODE
	strcat(bsend,"<input name=\"nrfrsp\" type=\"number\" value=\"");
...
//	if (s_retry_num >= (WIFI_MAXIMUM_RETRY << 6)) floop = 0;
