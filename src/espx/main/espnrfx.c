...
//	if (topoff && f_snfdel) {
	...
	} else if (NAMONOF2[0] && (!memcmp(event->topic+topoff, NAMONOF2, event->topic_len-topoff))) {
        t_tsonof = 2500;
	if ((!incascmp("1",event->data,event->data_len)) || (!incascmp("on",event->data,event->data_len))
		|| (!incascmp("true",event->data,event->data_len))) {
        if (!f_snfsnd2 || !gpio_get_level(RELAY2)) {
...
*/
	...
	itoa(gpio_get_level(BUTTON1),buff,10);
	strcat(bsend, buff);
	strcat(bsend, "-");
	itoa(gpio_get_level(BUTTON2),buff,10);
	strcat(bsend, buff);
	strcat(bsend, "/");
	itoa(gpio_get_level(RELAY1),buff,10);
	strcat(bsend, buff);
	strcat(bsend, "-");
	itoa(gpio_get_level(RELAY2),buff,10);
	strcat(bsend, buff);
...
//	strcat(bsend,"</span></a><a class='menu' href='livedata'>&#10740;<span class='showmenulabel'>Live Data");
	...
	itoa(TmZn,buff,10);
	strcat(bsend,buff);
	strcat(bsend,"\" min=\"-12\" max=\"12\" size=\"3\">GMT Timezone</br><h3>Relay Setting</h3></br><input name=\"ssnf1id\" value=\"");
	if (NAMONOF1[0]) strcat(bsend,NAMONOF1);
        strcat(bsend,"\" size=\"32\">Relay1 Name &emsp;<input name=\"ssnf1rp\" type=\"number\" value=\"");
	itoa(RELAY1,buff,10);
	strcat(bsend,buff);
	strcat(bsend,"\" min=\"0\" max=\"16\" size=\"2\">Relay1 pin &emsp;<input name=\"ssnf1bp\" value=\"");
	itoa(BUTTON1,buff,10);
	strcat(bsend,buff);
	strcat(bsend,"\" min=\"0\" max=\"16\" size=\"2\">Button1 pin</br></br><input name=\"ssnf2id\" value=\"");
	if (NAMONOF2[0]) strcat(bsend,NAMONOF2);
        strcat(bsend,"\" size=\"32\">Relay2 Name &emsp;<input name=\"ssnf2rp\" type=\"number\" value=\"");
	itoa(RELAY2,buff,10);
	strcat(bsend,buff);
	strcat(bsend,"\" min=\"0\" max=\"16\" size=\"2\">Relay2 pin &emsp;<input name=\"ssnf2bp\" value=\"");
	itoa(BUTTON2,buff,10);
	strcat(bsend,buff);
	strcat(bsend,"\" min=\"0\" max=\"16\" size=\"2\">Button2 pin</br></br>");
...
*/
	...
	parsuri(buf1,buf3,buf2,1024,4);
        pintemp = atoi(buf3);
	if (pinvalid(pintemp)) RELAY1 = pintemp;
	strcpy(buf2,"ssnf2rp");
	parsuri(buf1,buf3,buf2,1024,4);
        pintemp = atoi(buf3);
	if (pinvalid(pintemp)) RELAY2 = pintemp;
	strcpy(buf2,"ssnf1bp");
	parsuri(buf1,buf3,buf2,1024,4);
        pintemp = atoi(buf3);
	if (pinvalid(pintemp)) BUTTON1 = pintemp;
	strcpy(buf2,"ssnf2bp");
	parsuri(buf1,buf3,buf2,1024,4);
        pintemp = atoi(buf3);
	if (pinvalid(pintemp)) BUTTON2 = pintemp;
...
//state monitoring and command execution loop
    ...
    } else vTaskDelay(148 / portTICK_PERIOD_MS);
	...
	} else if (f_snfst1 || (!t_tsonof && !snfnum)) {
	if (NAMONOF1[0]) {
	...
	strcat(bufferln1," ");
	(gpio_get_level(RELAY1))? strcpy(MQTT_DATA,STRON1):strcpy(MQTT_DATA,STROF1);
	...
	t_tsonof = 2500;
	snfnum = 1;
	} else if (f_snfst2 || (!t_tsonof && snfnum)) {
	if (NAMONOF2[0]) {
	strcpy(MQTT_TOPIC,MQTT_BASE_TOPIC);
	strcat(MQTT_TOPIC,"/");
	...
	strcpy(bufferln1,MQTT_TOPIC);
	strcat(bufferln1," ");
	(gpio_get_level(RELAY2))? strcpy(MQTT_DATA,STRON2):strcpy(MQTT_DATA,STROF2);
	strcat(bufferln1,MQTT_DATA);
	if (mqttConnected) {
	esp_mqtt_client_publish(mqttclient, MQTT_TOPIC, MQTT_DATA, 0, 1, 1);
	f_snfsnd2 = true;
