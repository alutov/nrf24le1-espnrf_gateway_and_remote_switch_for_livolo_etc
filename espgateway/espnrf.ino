...
char MQTT_USER[MQTT_USER_LEN];		                         // "esp" MQTT Server user
char MQTT_PASSWORD[MQTT_PASSWORD_LEN];                 	   // "esp" MQTT Server password
char MQTT_SERVER[MQTT_SERVER_LEN];          		           // "192.168.1.10" MQTT Server
char WIFI_SSID[WIFI_SSID_LEN];                  	         // "Lutov_A_N_2" network SSID for ESP8266 to connect to
char WIFI_PASSWORD[WIFI_PASSWORD_LEN];         		         // "LAN_22121967" password for the network above
...
//      htmlMessage += "submitForm(); return false;\">" + String(USER_CMDs[i].text) + "</a>\r\n";  
   ...
   // System Info
    ...
    htmlMessage += "<tr><td style='min-width:150px;'>Version</td><td style='width:80%;'>2019.08.08</td></tr>\r\n";
...
void loop() {
        ...
        // handle lost of MQTT connection : next retry after 30s
        if (!MQTTClient.connected()) {
                if (t_mqtcon == 0) {
                n_mqtcon ++;
                DEBUG_PRINT(F("No MQTT, "));
                DEBUG_PRINT(n_mqtcon);
                DEBUG_PRINTLN(F(" attempt, retrying in 30s"));
                t_mqtcon = 60;
                MqttConnect();
	                  }
